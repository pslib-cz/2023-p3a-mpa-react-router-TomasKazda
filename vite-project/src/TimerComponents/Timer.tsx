      ...
      myInterval = window.setInterval(() => {
        if (testTime > 0) {
          ...
          if (
            ...
            timeLeftMinutes <= ThresholdValue
          ) {
            progressBar.current?.classList.add(
              styles.time__progress_bar_critical
            );
          } else if (
            ThresholdType === "percentage" &&
            timeLeftPercentage <= ThresholdValue
          ) {
            progressBar.current?.classList.add(
              styles.time__progress_bar_critical
            );
          } else {
            progressBar.current?.classList.remove(
              styles.time__progress_bar_critical
            );
          }
        }
